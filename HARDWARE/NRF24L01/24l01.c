...
//接收地址1 才发现低位在前，高位在后。低位优先，写的时候低位写到高位，读的时候低位先被读出
const u8 RX_ADDRESS_0[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址0
const u8 RX_ADDRESS_1[RX_ADR_WIDTH]={0x35,0x43,0x10,0x10,0x01}; //接收地址1
const u8 RX_ADDRESS_2[RX_ADR_WIDTH]={0x36,0x43,0x10,0x10,0x01}; //接收地址2
const u8 RX_ADDRESS_3[RX_ADR_WIDTH]={0x37,0x43,0x10,0x10,0x01}; //接收地址3
const u8 RX_ADDRESS_4[RX_ADR_WIDTH]={0x38,0x43,0x10,0x10,0x01}; //接收地址4
const u8 RX_ADDRESS_5[RX_ADR_WIDTH]={0x39,0x43,0x10,0x10,0x01}; //接收地址5

/*
u8 RX_ADD_0[RX_ADR_WIDTH]={0x34,0x43,0x10,0x10,0x01}; //接收地址0
u8 RX_ADD_1[RX_ADR_WIDTH]={0x35,0x43,0x10,0x10,0x01}; //接收地址1
u8 RX_ADD_2[RX_ADR_WIDTH]={0x36,0x43,0x10,0x10,0x01}; //接收地址2
u8 RX_ADD_3[RX_ADR_WIDTH]={0x37,0x43,0x10,0x10,0x01}; //接收地址3
u8 RX_ADD_4[RX_ADR_WIDTH]={0x38,0x43,0x10,0x10,0x01}; //接收地址4
u8 RX_ADD_5[RX_ADR_WIDTH]={0x39,0x43,0x10,0x10,0x01}; //接收地址5
*/
...
{ 	
	...
	GPIO_ResetBits(GPIOG,GPIO_Pin_6|GPIO_Pin_7);//PG6,7,8上拉					 
...
u8 NRF24L01_Check(void)
{
	...
	NRF24L01_Write_Buf(WRITE_REG_NRF+TX_ADDR,buf,5);//写入5个字节的地址.	  设置发送端地址
...
{
		NRF24L01_CE=0;	  //设置24L01为关机模式，用于修改无线配置	
  	NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P0,(u8*)RX_ADDRESS_0,RX_ADR_WIDTH);//写RX0节点地址
		NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P1,(u8*)RX_ADDRESS_1,RX_ADR_WIDTH);//写RX1节点地址
		NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P2,(u8*)RX_ADDRESS_2,1);//写RX2节点地址（只写1次，原因高位都被锁了）
		NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P3,(u8*)RX_ADDRESS_3,1);//写RX3节点地址
		NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P4,(u8*)RX_ADDRESS_4,1);//写RX4节点地址
		NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P5,(u8*)RX_ADDRESS_5,1);//写RX5节点地址
		
		/*
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P0,RX_ADD_0,RX_ADR_WIDTH);	 //查地址确定已写入
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P1,RX_ADD_1,RX_ADR_WIDTH);	 //查地址确定已写入
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P2,RX_ADD_2,1);	 //查地址确定已写入
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P3,RX_ADD_3,1);	 //查地址确定已写入
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P4,RX_ADD_4,1);	 //查地址确定已写入
		NRF24L01_Read_Buf(READ_REG_NRF+RX_ADDR_P5,RX_ADD_5,1);	 //查地址确定已写入
		*/
  	...
  	NRF24L01_Write_Reg(WRITE_REG_NRF+RX_PW_P0,RX_PLOAD_WIDTH);//选择通道0的有效数据宽度 	
		...
		/*2M 传输速率下 15 米，1M 传输速率 30 米，250K 传输速率 50 米。*/
...
{														 
  	...
  	NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P0,(u8*)RX_ADDRESS_0,RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK	  
