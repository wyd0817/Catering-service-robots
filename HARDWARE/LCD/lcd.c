...
void LCD_ShowChinese(u16 x,u16 y,u8 num,u8 mode)
{  							  
  u8 temp,t1,t;
	u16 y0=y;
	u16 colortemp=POINT_COLOR;      			     
	...
	if(!mode) //非叠加方式
	{
	    for(t=0;t<32;t++)
	    {   
				temp=oled_chinese_1616[num][t];  
				for(t1=0;t1<8;t1++)
			{			    
		    if(temp&0x80) POINT_COLOR=BACK_COLOR;
...
//size:字体大小
//*p:字符串起始地址		
void LCD_ShowChinese_num(u16 x,u16 y,u16 num,u8 mode)
{ 
#define MAX_CHINESE_POSX (320 - 16)
#define MAX_CHINESE_POSY (240 - 16) 
	u8 temp_num = 0;         
	while(temp_num != num)
	{       
		if(x>MAX_CHINESE_POSX){x=0;y+=16;}
		if(y>MAX_CHINESE_POSY){y=x=0;LCD_Clear(WHITE);}
		LCD_ShowChinese(x,y,temp_num,mode);	 
		x+=16;
		temp_num++;
	}  
}
